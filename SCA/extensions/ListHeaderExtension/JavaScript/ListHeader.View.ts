...
export const ListHeaderView: any = BackboneView.extend({
    ...
    events: {
        ...
        'change [data-action="perpage"]': 'pageHandler',
        'click [data-action="toggle-sort"]': 'toggleSortHandler',
        'keyup [data-action="searchInputitem"]': 'submitSearch',
        'click [data-action="searchInputicon"]': 'submitSearchIcon',
    ...
    getContext: function() {
        ...
        // let sli  = url.slice(11, url.indexOf('&') );
        // this.tr = sli.split('search=').join('') ;
        // var  sli = url.split('placeorder?show=2&').join('') ;
        // var tr = sli.slice(7,sli.indexOf('&'));
        // var ifSrc = tr.slice(0,6)
        
        // getParameterByName(url: string, param_name: string)
        ...
        if (this.selectedPage) {
            selected_page = this.selectedPage.value;
        }
    _.each(this.displays, function(display: any) {
        ...
        _.each(this.ShowperPage, function(show: any) { 
            // console.log(show);
               
            show.selected = selected_page === show.value;
        });

        var searchBy =  this.Searchurl ?  this.Searchurl:  this.SearchVal ;            // console.log(this.showNarrow,'this.showNarrow');
        ...
        return {
            ...
            noSearchFound:this.collection.nodata,
            ...
            // @property {boolean} show search bar
            searchItem:this.Showsearch,
            // @property {string } search by word
            SearchPar:searchBy,
    ...
    setSelecteds: function() {
        ...
        this.selectedPage = this.getShowPageFromUrl(url_options.show);
            ...
            collection.update(
                {
                    ...
                    ShowperPage:this.selectedPage,
                    // @property {String} order
                    order: this.order,
                    // @property {String} search
                    search:this.search  || this.SearchVal,
                    page: this.page,
    ...
    // @method getPageVal @param {String} value @returns {Object} returns a specific PAGE VALUE
    getPageVal: function(value) {
        return _.findWhere(this.ShowperPage, { value: value });
    },
    ...
    getShowPageFromUrl: function(url_value) {    
        return this.getPageVal(url_value) || this.getDefaultShowPage();
    },
    ...
    getDefaultShowPage: function() {
        return (
            this.defaultShow ||
            (this.defaultShow = _.findWhere(this.ShowperPage, { selected: true }) || _.first(this.ShowperPage))
        );
    },
    ...
    // @method isDefaultSort @returns {Boolean}
    isDefaultShow: function(show) {
        return this.getDefaultShowPage() === show;
    },
    ...
    pageHandler: function(e) {
        // sets the selected sort
        this.selectedPage = this.getPageVal(e.target.value);
        // updates the url and the collection
        this.updateUrl();
    },
    submitSearch :function(e){
        e.preventDefault();
        var inputVal = $('#searchInputitem').val();
        if (e.keyCode === 13 && !_.isEmpty(inputVal ) && inputVal != " " ) {
        sessionStorage.setItem('src',inputVal.toString())
        this.search  =  inputVal;
        this.Searchurl = this.search.replace(/ /g,'');
        this.updateUrl();
        }
        
    },
    submitSearchIcon:function(){
        var inputVal = $('#searchInputitem').val();
        if (!_.isEmpty(inputVal ) && inputVal != " " ) {
        sessionStorage.setItem('src',inputVal.toString())
        this.search  =  inputVal;
        this.Searchurl = this.search.replace(/ /g,'');
        this.updateUrl();
    ...
    updateUrl: function() {
        ...
        // search for items 
        // url = UrlHelper.setUrlParameter(url,'search',srcUrl);
        ...
        /************ */
        // url = this.isDefaultShow(this.selectedPage)
        //     ? UrlHelper.removeUrlParameter(url, 'show')
        //     : UrlHelper.setUrlParameter(url, 'show', this.selectedPage.value);


        url =  UrlHelper.setUrlParameter(url, 'show', this.selectedPage.value);
        // search url forming 
        // this.SearchVal
        this.Searchurl =  this.Searchurl   || this.SearchVal
        url = UrlHelper.setUrlParameter(url, 'search',this.Searchurl)
